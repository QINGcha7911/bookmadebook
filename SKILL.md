...
## 十、Agnes 免费 AI 动态开篇（v2.5 新增）

> **需求背景**：用户要"真实动态画面 + 免费 + 年代相符"。免费素材库（Mixkit/Pexels）全是现代拍摄；历史影像源（archive.org/Wikimedia）被墙。**Agnes AI（免费 key）生成纯场景动态画面**解决此死结。

### 10.1 前置
- Agnes 服务本地运行：`python server.py`（D:\AI软件\GitHub\agnes-video-generator，端口 8765）
- `.env` 配 `AGNES_API_KEY`（免费注册 platform.agnes-ai.com，GitHub 一键登录）
- 依赖：requests / easyocr（可选 OCR 校验）

### 10.2 工作流（scripts/agnes_intro.py）
```
讲书稿开头（60秒口播文案）
  → scenes_from_script() 拆场景（按句切分，每场景1-2句）
  → make_prompt() 生成 prompt（关键：纯场景+无人物+无文字+年代风格）
  → submit_simple() 提交 Agnes t2v 任务（duration=10，768x1152，audio/subtitle 关闭）
  → wait_task() 轮询完成 → 下载每段视频
  → ffmpeg concat 拼接（重编码 yuv420p）
  → 混入 edge-tts 云健中文口播（-shortest）
```

### 10.3 prompt 铁律（防翻车——实测教训）
| 问题 | 解法 |
|------|------|
| 画面太假（AI感）| 负面词：卡通/超现实/现代元素/高饱和 |
| 恐怖谷（人脸）| **纯场景空镜+远景大全景+无人物**（负面词硬禁人物/人脸/生物）|
| 英文字幕乱码（AI 会烧录伪英文）| prompt 中英文双重重禁：`NO TEXT, NO CAPTION, NO SUBTITLE, NO WATERMARK, NO SIGN, NO LETTERS` |
| 旁白是英文 | 提交参数 `audio_enabled=False, subtitle_enabled=False`（旁白用本地 edge-tts 云健替换）|
| duration 非法 | 仅支持 [5, 10, 15, 18, 20] |
| 提交 422 | multipart 必须带 `files={'x': ('', '')}` |

### 10.4 成品规格
- 60 秒 = 5-6 段 × 10 秒（场景不足时末段 tpad 延长）
- 竖版 768×1152（Agnes 输出 832×1088 左右）
...
└── scripts/                    ← 工具脚本（不要修改，共 21 个）
    ...
    ├── agnes_intro.py                                           ← 60秒开篇·Agnes免费AI动态画面（0元）
