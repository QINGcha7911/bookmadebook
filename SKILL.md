...
description: |-
  ...
  视频模式（可选）：60秒开篇钩子视频 + 精读正文（可灵AI画面/字卡/金句）。
...
## 〇、视频模式工作流（2026-08-18 用户固化设定 v2.3.1）

> 精读以**音频为主、视频为辅**。生成精读视频时（默认开启）：
> 1. **60 秒开篇视频**：钩子开场（悬念/金句）+ 书名 + 核心亮点，节奏快、竖版 1080×1920
> 2. **接着精读正文**：正文画面（字卡/图片/实景场景）随讲书稿章节推进
> 3. **画面策略（v2.3.1 免费版为默认）**：实景素材 Ken Burns 运镜（assets/scenes 库）+ 金句字卡 + 本地口播 = **0 元成本**；**可灵仅在爆款/重点书使用**（≤2 段，成本 20-40 元）——非必需
> 3.1 **画面风格按内容自动适配（用户设定）**：
>   - **历史传记/战争/年代类**（历史/传记/战史/人物）：实景图**灰度化**（黑白质感——庄重纪实）
>   - **惬意抒情/生活/治愈类**（散文/治愈/生活/情感）：保持**彩色**（温暖明亮）
>   - 判定依据：讲书稿内容类型检测（历史关键词命中→黑白，否则彩色）
